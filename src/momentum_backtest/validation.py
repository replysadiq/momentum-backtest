...
def validate_weights_sum(
    ...
    V4: Breadth scaling is applied during return calculation, NOT to stored weights.
         Weights always sum to 1.0 when invested, final_exposure is used for scaling.
    ...
    """
    # States where we should be invested (weights sum to 1.0 when stocks selected)
    ...
    for record in rebalance_records:
        ...
        if is_invested:
            ...
            if n_stocks > 0:
                # Weights sum to 1.0 (breadth scaling is applied during return calc)
                expected = 1.0
